...
def consulta_por_mes(data_inicio, data_fim):
    ...
    FROM SIGITM3.TBL_TA_VIDA VD2
         INNER JOIN SIGITM3.TBL_GRUPOS GRP2 ON VD2.VDA_RESPONSAVELPOR_GRUPO = GRP2.GRP_CODIGO
         WHERE VD2.VDA_RESPONSAVELPOR_GRUPO IN ('6611', '6612')) VD2 ON VD2.VDA_TA = TA.TQA_CODIGO
    WHERE TA.TQA_ORIGEM IS NULL
    AND TA.TQA_TIPO_REDE IN ('701')
    -- AND TA.TQA_CODIGO IN ('358333914', '358338454', '358326681')
    ...
    SELECT
        ...
        CASE
            WHEN B.VDA_RESPONSAVELPOR_USUARIO IS NOT NULL
                 AND (B.USUARIO_ANTERIOR IS NULL
...
# import concurrent.futures
# import pandas as pd
# from sqlalchemy import create_engine, text  # se estiver usando SQLAlchemy
...
def gerar_intervalos(inicio, fim, tipo='diario'):
    intervalos = []
    atual = inicio  # ✅ mantém hora original
    ...
    while atual < fim:
        ...
        intervalos.append((
            ...
            min(proximo, fim).strftime('%Y-%m-%d %H:%M:%S')
        ))
...
def consulta_por_mes(data_inicio, data_fim):
    ...
    AND TA.TQA_DATA_CRIACAO >= TO_DATE('2026-07-01','YYYY-MM-DD')
...
# Período total
# data_inicio = datetime(2026, 7, 1)
# data_inicio = data_inicio #data referente a data_inicio no começo do código
# data_atual = datetime.now()
# data_atual = datetime(2026, 7, 11)
# Intervalos diários
periodos = gerar_intervalos(data_inicio, data_atual, tipo=tipo_ciclo)
...
for inicio, fim in periodos:
    try:
        df = consulta_por_mes(inicio, fim)
 
        if df is not None and not df.empty:
            resultados.append(df)
 
    except Exception as e:
        print(f"Erro: {inicio} | {e}")
...
# n2_bloqueio = n2_bloqueio.drop(columns=['data_entrada_n2', 'data_primeiro_usuario_n2'])
...
print(
    "FINAL:",
    n2_consolidado_fechados.shape
)



n2_consolidado_fechados


#exportar unificado
# arquivo_saida = (
#     f"BASE_2_MESES_VALIDACAO_N2_v3"
...
#     n2_consolidado_ativos.to_excel(
#         writer,
#         sheet_name="ATIVOS",
#         index=False
#     )

#     n2_consolidado_fechados.to_excel(
#         writer,
#         sheet_name="FECHADOS",
#         index=False
#     )
